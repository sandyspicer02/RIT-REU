...
def obtain_halos_by_mass(lower, upper, snap, limit=100):
    """
    ...
    print(' Obtaining subhalos in mass range %g to %g at snapshot %f' % (lower, upper, snap))
    
    # First, obtain the initial batch
    search_query = "?limit=" + str(limit) + "&mass__gt=" + str(lower) + "&mass__lt=" + str(upper)
    url = "http://www.illustris-project.org/api/Illustris-1/snapshots/%i/subhalos/%s" % (snap, search_query)
    partial_resp = get(url)
    if DEBUG: print('count = %i len = %i' % (partial_resp['count'], len(partial_resp['results'])))
    final_resp = partial_resp
    print('   %i subhalos will be obtained in %i-element chunks' % (final_resp['count'], limit))
    while partial_resp['next'] is not None: # We don't have the full list...
        new_partial_resp = get(partial_resp['next'])
        final_resp['results'] = final_resp['results'] + new_partial_resp['results']
        partial_resp = new_partial_resp
        if DEBUG: print('count = %i len = %i cumulative = %i' % (partial_resp['count'], len(partial_resp['results']), len(final_resp['results'])))
    
    if len(final_resp['results']) != final_resp['count']:
        print(' ERROR: mismatch between the count field -- %i -- and the length of the results list -- %i' %(len(final_resp['results']), final_resp['count']))
        sys.exit(12)
        
    # Remove the now useless next/previous URIs
    del(final_resp['next'])
    del(final_resp['previous'])
    return final_resp

def obtain_subhalo_data(idn, snap, z):
    """
    ...
    """
    url = "http://www.illustris-project.org/api/Illustris-1/snapshots/%i/subhalos/%s" % (snap, idn)
    json = get(url)
    add_metadata(json, 'redshift', z)
    return json

def obtain_subhalos_data(resp, snap, z, limit=None):
    """
    Get metadata for a particular subhalo
    ...
    i = 0
    for idn in ids:
        dat.append(obtain_subhalo_data(idn, snap, z))
...
def filter_non_zero_mass(halos_data, ids, arg=None):
    ...
    for i in range(len(ids)):
        halo = halos_data[i]
        if halo['mass_gas'] < epsilon or halo['mass_stars'] < epsilon or halo['sfr'] < epsilon:
...
def obtain_cutout(halo_data, dmcols=None, starcols=None, gascols=None, bhcols=None):
    """
    ...
    """
    idn = halo_data['id']
    print(' Obtaining cutout of halo %i at redshift %f' % (idn, halo_data['redshift']))
    ...
    full_url = halo_data['cutouts']['subhalo']
    
    if DEBUG: print(' URI = %s' % full_url)
    ...
    transpose_cutout(outname, idn, ptypes=(0,4), comptypes=(4,))
    
    add_metadata(halo_data, 'downloaded_cutouts', {'subhalo': outname, 'subhalo_original': copyname})
    
    return outname

def obtain_cutouts(halos_data, dmcols=None, starcols=None, gascols=None, bhcols=None, count=None):
    """
    ...
    r = []
    if count is None:
        count = len(halos_data)
        print('Cutouts will be obtained for %i halos' % count)
    for i in range(count):
        r.append(obtain_cutout(halos_data[i], dmcols=dmcols, starcols=starcols, gascols=gascols, bhcols=bhcols))
    return r
...
def detilt_orig(fname, ptypes=(0, 4), angmomptypes=(4,), L_output=None):
    """
    ...
    if type(L_output) is list:
        L_output.append(totL)
        L_output.append(modL)
        L_output.append(Lhat)
    if DEBUG: print(Lhat)
...
def detilt(fname, halo_data, ptypes=(0, 4), angmomptypes=(4,), rlim_func=get_stellar_halfmass, L_output=None):
    """
    ...
    if DEBUG and type(L_output) is list: print(L_output)
...
def detilt_files(resps, halos_data, ptypes=(0, 4), detilt_func=detilt, L_outputs=None):
    """
    The new, rotation matrix based, detilt algorithm. USE THIS ONE.
    ...
    for i, f in enumerate(resps):
        if type(L_outputs) is list:
            L_output = [] # Do this every time so the list gets cleared.
        detilt_func(f, halos_data[i], ptypes=ptypes, L_output=L_output)
        if type(L_outputs) is list:
            L_outputs.append(L_output)
    if DEBUG: print(' L data len : %i'%len(L_outputs))
    
def store_metadata(halos_data, fname_prefix='data_', fname_suffix='.pickle', ftype='pickle'):
    """
    ...
    """
    if ftype == 'pickle':
        pickle_metadata(halos_data, fname_prefix='data_', fname_suffix='.pickle')

def pickle_metadata(halos_data, fname_prefix='data_', fname_suffix='.pickle'):
    """
    ...
    try:
        import cPickle # Python2 support
        if DEBUG: print(' No cpickle?!')
    except ImportError:
        import pickle
    
    proto = pickle.HIGHEST_PROTOCOL
    # If you need python2 to be able to read files produced by python3, uncomment the following line
    #proto = 2
    
    print(' Beginning pickle process at protocol version %i '%proto)
    
    for halo in halos_data:
        idn = halo['id']
        if DEBUG: print('   %i'  % idn)
        fname = '%s%i%s' % (fname_prefix, idn, fname_suffix)
        print(' Pickling %i to %s.' % (idn, fname))
        f = open(fname, mode='wb')
        pickle.dump(halo, f, protocol=proto)
        
        
def load_metadata(files, ftype='pickle'):
    """
    ...
    if ftype == 'pickle':
        halos_data = unpickle_metadata(files)
    
    return halos_data

def unpickle_metadata(files):
    """
    ...
    print(' Beginning unpickle process...')
    ...
    for fname in files:
        ...
        halo = pickle.load(f, encoding='ASCII') # Switch enconding to 'bytes' if python3 whinges when importing pickles from python2 (but you really shouldn't be doing that anyway)
        print(' UnPickled %i from %s '% (halo['id'], fname))
        f.close()
        halos_data.append(halo)
    return halos_data

def add_metadata(json, key, data):
    """
    ...
    """
    if key in json:
        print(" ERROR: attempt to add metadata key %s which already exists" % key)
        sys.exit(13)
    json[key] = data
